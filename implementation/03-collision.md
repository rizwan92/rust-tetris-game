...
Finish the collision feature in:

- `src/collision.rs`

This guide assumes baseline and config are already done.

## 1. Replace `there_is_collision`

Paste this function into `src/collision.rs`:
...
```bash
cargo nextest run --features test --retries 0 --test-threads=1 --test end_to_end basic_game_over basic_stacking --no-fail-fast
